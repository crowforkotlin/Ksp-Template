...
package com.crow.ksp.processor
...
class PathProcessor(private val environment: SymbolProcessorEnvironment) : SymbolProcessor, KSPLogger by environment.logger {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        // 获取序列（集合）
        val symbols: Sequence<KSAnnotated> = resolver.getSymbolsWithAnnotation(Path::class.java.name)
        ...
        symbols.filterIsInstance<KSClassDeclaration>()
            .forEach { ksClass -> println("[Processor] -> ksClass simpleName is ${ksClass.simpleName} \t ${ksClass.packageName}") }
